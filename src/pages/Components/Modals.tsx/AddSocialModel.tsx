...
export const AddSocialMediaModal: React.FC<ModalProps> = ({ isOpen, onClose, onCreated }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await client.post('/api/v1/dashboard/create_social_media/', data, {
      ...
      if (error instanceof Error && 'response' in error) {
        ...
        if (axiosError.response?.data) {
          if ('detail' in axiosError.response.data) {
            toast.error(axiosError.response.data.detail || "Saqlashda xatolik");
          } else {
